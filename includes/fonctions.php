...
function pagination($total, $page_actu = 1, $par_pg = 20, $var_get = "pg") {
    $get_admin=isset($_GET['a'])?'&a='.$_GET['a']:'';
        ...
        for ($i = 1; $i <= $nombre_pg; $i++) {
            if ($i == 1) {
                if ($i == $page_actu) {
                    ...
                    $sortie.= "<a href='?menu=".$_GET['menu'].$get_admin."&$var_get=$i'><<</a> <a href='?menu=".$_GET['menu'].$get_admin."&$var_get=" . ($page_actu - 1) . "'><</a> ";
                }
            }
            if ($i != $page_actu) {
                $sortie .= "<a href='?menu=".$_GET['menu'].$get_admin."&$var_get=$i'>$i</a>";
            } else {
                $sortie .= " $i ";
            }
            if ($i != $nombre_pg) {
                ...
                if ($i == $page_actu) {
                    ...
                    $sortie.= " <a href='?menu=".$_GET['menu'].$get_admin."&$var_get=".($page_actu + 1) . "'>></a> <a href='?menu=".$_GET['menu'].$get_admin."&$var_get=$nombre_pg'>>></a> ";
                }
            }
        }
        return $sortie;
